...
class Organism:
    ...
    def fitness(self, concentrations_list, mat, target_list, active_states=None):
        if len(concentrations_list) == 0 or len(target_list) == 0:
            raise ValueError('No values to calculate fitness for!')
        if self.total_fitness is not None:
    ...
    '''
    def mate(self, organism, div=3.0):
        ...
        w = [w[i] + np.random.normal(loc=0, scale=abs(self.weights[i]-organism.weights[i])/div) for i in range(len(w))]
    ...
    def refine(self, iterations=30):
        for i in range(1,iterations+1):
            ...
            for org in self.organisms:
                #new_orgs.append(org.mate(np.random.choice(self.organisms, p=p), div=float(i)))
